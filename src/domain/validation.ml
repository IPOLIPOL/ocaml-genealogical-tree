...
let string_of_error = function
  | Empty_individual_id id ->
      "empty individual id: " ^ id

  | Duplicate_individual_id id ->
      "duplicate individual id: " ^ id

  | Empty_relation_id id ->
      "empty relation id: " ^ id

  | Duplicate_relation_id id ->
      "duplicate relation id: " ^ id

  | Unknown_individual id ->
      "unknown individual: " ^ id

  | Self_parent id ->
      "individual cannot be their own parent: " ^ id

  | Self_spouse id ->
      "individual cannot be their own spouse: " ^ id

  | Birth_after_death id ->
      "birth date is after death date: " ^ id

  | Parent_born_after_child { parent; child } ->
      Printf.sprintf
        "parent %s is born after child %s"
        parent
        child
...
let validate_individuals (individuals : individual list) =
  ...
  List.iter
    (fun (individual_value : individual) ->
      if String.trim individual_value.id = "" then
        errors := Empty_individual_id individual_value.id :: !errors;
      if Hashtbl.mem seen individual_value.id then
        errors := Duplicate_individual_id individual_value.id :: !errors
      else
        Hashtbl.add seen individual_value.id ();
      match
        year_of_date individual_value.birth_date,
        year_of_date individual_value.death_date
      with
      | Some birth, Some death when birth > death ->
          errors := Birth_after_death individual_value.id :: !errors
      | _ -> ())
    individuals;
  List.rev !errors
...
let validate_references
  ...
  let known =
    Hashtbl.create (List.length individuals)
  in
  ...
  let require_individual individual_id =
    if not (Hashtbl.mem known individual_id) then
      errors :=
        Unknown_individual individual_id
        :: !errors
  in
    ...
    (function
      | Parent_child parent_child ->
          ...
          if parent_child.parent = parent_child.child then
            errors :=
              Self_parent parent_child.parent
              :: !errors

      | Spouse spouse ->
          require_individual spouse.individual1_id;
          require_individual spouse.individual2_id;

          if spouse.individual1_id = spouse.individual2_id then
            errors :=
              Self_spouse spouse.individual1_id
              :: !errors)
    relations;
...
let validate_parent_chronology
  ...
  List.iter
    (fun (individual_value : individual) ->
      Hashtbl.replace by_id individual_value.id individual_value)
    individuals;

  let errors = ref [] in

  List.iter
    (fun (relation_value : relation) ->
      match relation_value with
      | Parent_child parent_child
        when parent_child.confidence <> Rejected ->
          begin match
          ...
          | Some parent, Some child ->
              begin match
                year_of_date parent.birth_date,
                year_of_date child.birth_date
              with
              | Some parent_year, Some child_year
                when parent_year > child_year ->
                  errors :=
